...
| CRUD model actions | Generated model actions (`find_*`, `create`, `update`, `delete`, `upsert`, `count`, aggregates where currently exposed) retain Python signatures and return shapes. | Supported | Existing model-action suites pass against PostgreSQL JS bridge fixtures. | Signature drift, relation payload shape drift, or requiring JS query objects. |
...
| Interactive transactions | Preserve source-compatible `tx()` behavior through the bridge-hosted Prisma interactive transaction command loop. | Supported | Start/query/commit/rollback/timeout/cancellation/closed-ID fixtures pass for PostgreSQL before preview and for any later provider before it is marked supported. | Silent fallback, hanging transaction, root-client execution inside a transaction, or terminal transaction ID reuse. |
...
| Gate | PostgreSQL first release | Later SQLite path | Later MySQL/MariaDB path | Default-flip requirement |
| --- | --- | --- | --- | --- |
| CRUD parity | Required for self-hosted PostgreSQL before preview. | Deferred until separately approved. | Deferred until separately approved. | PostgreSQL required; all other providers must be explicitly deferred in release notes. |
| Scalar parity | Required for PostgreSQL native mappings, JSON, Decimal, Bytes, DateTime, and relation payloads. | Deferred until SQLite fixture schemas are added. | Deferred until Decimal/DateTime/provider mappings are validated. | Provider cannot be marked supported until scalar fixtures pass. |
| Transactions | Batch and interactive transaction suites are required; nested transactions remain explicit fail-fast unless savepoint fixtures pass. | Deferred until adapter-specific transaction fixtures pass. | Deferred until adapter pool and transaction semantics are validated. | Default flip blocked by any undocumented transaction divergence. |
| Raw queries | Partial until PostgreSQL parameter/result fixtures pass; otherwise explicit unsupported error. | Deferred. | Deferred. | Raw-query behavior cannot silently differ from existing Python API. |
| Migrate/introspection CLI | May remain delegated to Prisma CLI and existing Python CLI wrappers. | Same when provider is later enabled. | Same when provider is later enabled. | CLI smoke tests must prove JS bridge runtime does not break generation/setup. |
...
- [ ] CRUD, filter/order/select/include, model hydration, and scalar fixtures pass for PostgreSQL.
